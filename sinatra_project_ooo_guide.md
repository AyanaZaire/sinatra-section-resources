...
- [ ] 12. Create your `ApplicationController` helper methods (Why do we add this?)
...
- [ ] 15. Create your `PostsController`
  - Mount new controller in `config.ru` with `use PostsController` (Why do we add this?)
...
- [ ] 18. Implement UPDATE functionality
  - Add `use Rack::MethodOverride` in `config.ru` (Why do we add this?)
